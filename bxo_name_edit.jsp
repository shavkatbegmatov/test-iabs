...
<t:requests>
    <t:request name="save"><script><%
          ...
          try {
              ...
              //cs.setString(7,request.getParameter("created_on"));
              cs.execute();
              if (cs.getString(1).equals("OK")){%>
                alert("<%=cods.fixEnter(cs.getString(2))%>");parent.pageLock(false);top.returnValue={};top.close();<%
              } else {%>
                alert("<%=lang.get(si_error)%>");parent.pageLock(false);top.returnValue={};top.close();<%}
          } catch (Exception ex) {
...
<%!
    static final int si_form_title = SI("Филиалнинг номини тахрирлаш");
    ...
    static final int si_begin_date          = SI("Дата начало новой имени");
